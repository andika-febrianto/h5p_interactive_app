import type { Module, Subject, FrameResult, Frame, User, UserRole } from '../types/storyboard';
...
async function request<T>(path: string, options?: RequestInit, isRetry = false): Promise<T> {
  ...
  if (!res.ok) {
    ...
    throw new ApiError(body.error ?? `Permintaan gagal (${res.status})`, res.status, body.code);
  }
...
export function fetchProgressSummary(clientId: string): Promise<Record<string, number>> {
...
export function clearProgress(clientId: string, moduleId: string): Promise<void> {
  const qs = new URLSearchParams({ clientId, moduleId });
...
export function updateModule(id: string, body: Record<string, unknown>): Promise<Module> {
  return request(`/modules/${encodeURIComponent(id)}`, {
    method: 'PUT',
...
export function createFrame(moduleId: string, frame: Record<string, unknown>): Promise<Frame> {
...
export function updateFrame(
  ...
  return request(`/modules/${encodeURIComponent(moduleId)}/frames/${encodeURIComponent(slug)}`, {
    method: 'PUT',
    body: JSON.stringify(frame),
  });
}
...
export function reorderFrames(moduleId: string, order: string[]): Promise<void> {
...
export function cancelSubscription(): Promise<{ status: SubscriptionStatus; currentPeriodEnd: string }> {
...
export function createCheckout(planId: 'basic' | 'pro'): Promise<{ invoiceUrl: string }> {
  return request('/subscription/checkout', { method: 'POST', body: JSON.stringify({ planId }) });
}
...
export async function uploadFile(file: File): Promise<UploadResult> {
  const doUpload = async (isRetry = false): Promise<UploadResult> => {
    ...
    if (!res.ok) {
      ...
      throw new ApiError(body.error ?? `Unggah gagal (${res.status})`, res.status);
    }
