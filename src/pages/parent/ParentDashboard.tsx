...
const S = {
  ...
  brandIcon: {
    ...
    fontWeight: 700,
    boxShadow: '0 4px 12px rgba(91,77,255,0.2)',
  } as React.CSSProperties,
  brandTitle: {
    fontSize: 20,
    fontWeight: 800,
  ...
  tab: (active: boolean) =>
    ({
      ...
      cursor: 'pointer',
      border: 'none',
      background: active ? '#5B4DFF' : 'transparent',
  ...
  avatar: {
    ...
    height: 40,
    borderRadius: 12,
    background: 'linear-gradient(135deg, #5B4DFF, #6366F1)',
    color: '#fff',
    fontWeight: 700,
    display: 'flex',
    alignItems: 'center',
  ...
  hero: {
    ...
    overflow: 'hidden',
    background:
      'linear-gradient(135deg, #F5F3FF 0%, #EEF2FF 50%, #EDE9FE 100%)',
    border: '1px solid rgba(91,77,255,0.15)',
    boxShadow: '0 1px 3px rgba(0,0,0,0.04)',
  } as React.CSSProperties,
  heroInner: {
  ...
  card: {
    ...
    border: '1px solid #e2e8f0',
    boxShadow:
      '0 4px 20px -2px rgba(91,77,255,0.05), 0 2px 6px -1px rgba(0,0,0,0.02)',
  } as React.CSSProperties,
  cardHeader: {
    display: 'flex',
    justifyContent: 'space-between',
  ...
  progressCard: {
    padding: 20,
    borderRadius: 16,
    background: 'linear-gradient(135deg, #f8fafc, rgba(91,77,255,0.04))',
  ...
  perfCard: {
    background: 'linear-gradient(135deg, #1E1B4B, #2A1F6D, #0f172a)',
    borderRadius: 24,
    padding: 28,
    color: '#fff',
    position: 'relative' as const,
...
export default function ParentDashboard() {
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      setAssignSuccess(
        `✓ "${selectedModule.title}" (${selectedFrames.length} panel) berhasil ditugaskan ke ${childInfo?.name ?? 'anak'}!`,
      )
    ...
    async (childId: string) => {
      ...
      await Promise.all(
        childAssignments.map(async (a) => {
          try {
            allQ[a.id] = await fetchAssignmentQuestions(a.id)
          } catch {
            allQ[a.id] = []
          }
        }),
  ...
  const getAssignmentCompletion = (
    ...
    if (!a.selectedFrames || !a.materialId)
  ...
  const deadlineStatus = (a: ParentAssignment) => {
    ...
    if (hrs < 0)
      return {
        label: 'Perlu Perhatian / Terlambat',
        color: '#dc2626',
        bg: '#fef2f2',
        border: '#fecaca',
      }
        ...
        <div style={S.headerInner}>
          <div style={S.brandArea}>
            ...
            <div>
              <div style={S.brandTitle}>
                Perpustakaan <span style={{ color: '#5B4DFF' }}>Belajar</span>
              </div>
          ...
          <nav style={S.tabs}>
            {tabs.map((t) => (
              <button
                key={t.key}
                style={S.tab(viewMode === t.key)}
                onClick={() => setViewMode(t.key as ViewMode)}
              >
                <span>{t.icon}</span>
                <span>{t.label}</span>
              </button>
            ...
            {/* <div style={S.profileArea} onClick={() => setShowCreateChild(true)}>
              ...
              <div>
                ...
                <div style={{ fontSize: 11, color: '#94a3b8' }}>
                  Ayah dari {childName} ({childGradeText})
                </div>
              </div>
              <svg
                width='16'
        ...
        <div style={S.modalOverlay} onClick={() => setShowCreateChild(false)}>
          <div style={S.modalBox} onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleCreateChild}>
              ...
              {childError && (
                <p
                  style={{ color: '#dc2626', fontSize: 13, margin: '0 0 8px' }}
                >
                  {childError}
                </p>
              )}
              <button
                ...
                style={{ ...S.btnPrimary, opacity: childSubmitting ? 0.6 : 1 }}
              >
          ...
          <div style={S.modalBox} onClick={(e) => e.stopPropagation()}>
            ...
            {selectedModule && (
              <div style={{ marginBottom: 12 }}>
                ...
                <p
                  style={{ fontSize: 11, color: '#94a3b8', margin: '4px 0 0' }}
                >
            ...
            {selectedModule && selectedFrames.length > 0 && (
              <button
                ...
                style={{ ...S.btnPrimary, opacity: assigning ? 0.6 : 1 }}
              >
                {assigning
                  ? 'Menugaskan...'
        ...
        <div style={S.modalOverlay} onClick={() => setEditingAssignment(null)}>
          <div style={S.modalBox} onClick={(e) => e.stopPropagation()}>
            ...
            {editModule && editSelectedFrames.length > 0 && (
              <div style={{ marginBottom: 12 }}>
                <p
                  style={{ fontSize: 13, color: '#64748b', margin: '0 0 6px' }}
                >
