...
// ── Sample data for the report (static demo) ──
const DEMO_TASKS = [
  {
    id: '1',
    icon: '➗',
    ...
    statusBorder: '#A7F3D0',
    statusDot: '#10B981',
    note: 'Sangat mahir membaca nilai tempat ribuan. Dylan mampu menjawab 19 dari 20 soal dengan mandiri.',
    noteColor: C.slate600,
    actionBg: '#F5F3FF',
    actionColor: '#5B4DFF',
    actionHoverBg: '#EDE9FE',
  ...
  },
  {
    ...
    subtopic: 'Eksperimen Klorofil & Kuis Interaktif 10 Soal',
    date: '06 Sep 2026',
    duration: '30 Menit',
    score: 88,
    ...
    statusBorder: '#BFDBFE',
    statusDot: '#3B82F6',
    note: 'Konsep sinar matahari dan klorofil dipahami baik. Disarankan praktik melihat daun di taman rumah.',
    noteColor: C.slate600,
    actionBg: '#F5F3FF',
    actionColor: '#5B4DFF',
    actionHoverBg: '#EDE9FE',
  ...
  },
  {
    ...
    subtopic: 'Menemukan Gagasan Utama & Tanda Titik Koma',
    date: '04 Sep 2026',
    duration: '20 Menit',
    score: 90,
    status: 'Selesai Sempurna',
    ...
    statusBorder: '#A7F3D0',
    statusDot: '#10B981',
    note: 'Intonasi membaca semakin jelas dan percaya diri. Kosakata baru bertambah 6 kata.',
    noteColor: C.slate600,
    actionBg: '#F5F3FF',
    actionColor: '#5B4DFF',
    actionHoverBg: '#EDE9FE',
  ...
  },
  {
    ...
    subtopic: 'Spelling Quiz & Audio Listening Pasangan Kata',
    date: '02 Sep 2026',
    duration: '35 Menit',
    score: 75,
    ...
    statusBorder: '#FDE68A',
    statusDot: '#F59E0B',
    note: 'Perlu penguatan pada ejaan kata ganda (misal: "blackboard", "scissors"). Disarankan mengulang sesi mini kuis audio.',
    noteColor: '#92400E',
    actionBg: '#FEF3C7',
    actionColor: '#B45309',
    actionHoverBg: '#FDE68A',
  ...
  },
  {
    ...
    subtopic: 'Menghafal Urutan & Cerita Teladan Sahabat',
    date: '30 Agu 2026',
    duration: '18 Menit',
    score: 98,
    status: 'Selesai Sempurna',
    ...
    statusBorder: '#A7F3D0',
    statusDot: '#10B981',
    note: 'Hafalan 6 rukun iman lancar dan mengaitkannya dengan adab menghormati orang tua dan teman.',
    noteColor: C.slate600,
    actionBg: '#F5F3FF',
    actionColor: '#5B4DFF',
    actionHoverBg: '#EDE9FE',
...
const SUBJECT_PERF = [
  { icon: '➗', name: 'Matematika (Bilangan Cacah & Operasi)', pct: 92, label: 'Sangat Baik', color: C.brand600, bg: C.brand50, border: C.brand100, gradient: `linear-gradient(90deg, ${C.brand500}, ${C.indigo600})` },
  { icon: '🔬', name: 'IPAS (Makhluk Hidup & Siklus Air)', pct: 85, label: 'Baik Sekali', color: C.emerald600, bg: C.emerald50, border: C.emerald100, gradient: `linear-gradient(90deg, ${C.emerald500}, ${C.teal500})` },
  { icon: '📖', name: 'Bahasa Indonesia (Teks Narasi & Kosa Kata)', pct: 88, label: 'Baik Sekali', color: C.blue600, bg: C.blue50, border: C.blue100, gradient: `linear-gradient(90deg, ${C.blue500}, #06B6D4)` },
  { icon: '🕌', name: 'PAI & Budi Pekerti (Rukun Iman)', pct: 95, label: 'Sempurna', color: C.purple600, bg: C.purple50, border: C.purple100, gradient: `linear-gradient(90deg, ${C.purple600}, ${C.pink500})` },
  { icon: '🎨', name: 'Seni Budaya & Prakarya (SBdP)', pct: 90, label: 'Kreatif', color: C.amber500, bg: C.amber50, border: C.amber100, gradient: `linear-gradient(90deg, #FBBF24, ${C.orange500})` },
  { icon: '🇬🇧', name: 'Bahasa Inggris (Vocabulary & Phonics)', pct: 80, label: 'Cukup Baik', color: C.rose500, bg: C.rose50, border: C.rose100, gradient: `linear-gradient(90deg, ${C.rose400}, ${C.rose600})` },
]

export default function LaporanRapor(props: LaporanRaporProps) {
  const { children: childrenData, selectedChildIdx, assignments, subjects, modules: _modules, moduleCache: _moduleCache, assignmentProgress: _assignmentProgress } = props
  void _modules; void _moduleCache; void _assignmentProgress
  ...
  const totalCompleted = childAssignments.filter((a) => a.status === 'completed').length
  const totalAssignments = childAssignments.length || 24

  const avgScore = useMemo(() => {
    const scores = DEMO_TASKS.map((t) => t.score)
    return scores.reduce((a, b) => a + b, 0) / scores.length
  }, [])

  const completedModules = totalCompleted || 18
  const totalModules = totalAssignments || 22

  const childName = activeChild?.name?.split(' ')[0] ?? 'Dylan'
  const childFullName = activeChild?.name ?? 'Dylan Pratama'
  const childInitial = childName.charAt(0).toUpperCase()
  const childGrade = activeChild?.grade ?? 3

  const otherChildName = otherChild?.name?.split(' ')[0] ?? 'Zaki'
  const otherFullName = otherChild?.name ?? 'Zaki Al-Fatih'
  const otherInitial = otherChildName.charAt(0).toUpperCase()
  const otherGrade = otherChild?.grade ?? 1

  const otherCompleted = (() => {
    const ca = assignments.filter((a) => a.childId === otherChild?.id)
    return ca.filter((a) => a.status === 'completed').length
  })()
  const otherTotal = assignments.filter((a) => a.childId === otherChild?.id).length || 15
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 28, fontFamily: FF }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', gap: 24, flexWrap: 'wrap' as const }}>
          <div style={{ maxWidth: 600 }}>
            ...
            <p style={{ fontSize: 14, color: C.slate500, margin: 0, lineHeight: 1.6, maxWidth: 540 }}>
              Pantau grafik performa pemahaman materi, durasi belajar mingguan, serta rekapitulasi nilai tugas harian {childName} dan {otherChildName} secara mendalam.
            </p>
          </div>
        ...
        <div style={{ background: C.white, borderRadius: 20, padding: 20, border: `2px solid ${C.brand600}`, boxShadow: '0 4px 16px rgba(91,77,255,0.12)', display: 'flex', flexDirection: 'column' as const, justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 11, color: C.slate500, margin: '3px 0 0' }}>Kelas {childGrade} SD • Kurikulum Merdeka</p>
              </div>
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 16, paddingTop: 16, borderTop: `1px solid ${C.slate100}`, fontSize: 11 }}>
            <div>
              ...
              <span style={{ fontWeight: 800, color: C.brand600, fontSize: 14 }}>{avgScore.toFixed(1)} / 100</span>
            </div>
            <div>
              <span style={{ color: C.slate500, display: 'block' }}>Tugas Selesai:</span>
              <span style={{ fontWeight: 800, color: C.emerald600, fontSize: 14 }}>{completedModules} / {totalModules} Tugas</span>
            </div>
          </div>
        </div>

        {/* Child 2: Inactive */}
        <div style={{ background: 'rgba(255,255,255,0.7)', borderRadius: 20, padding: 20, border: `1px solid ${C.slate200}`, boxShadow: '0 1px 3px rgba(0,0,0,0.04)', display: 'flex', flexDirection: 'column' as const, justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 11, color: C.slate500, margin: '3px 0 0' }}>Kelas {otherGrade} SD • Kurikulum Merdeka</p>
              </div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: C.amber50, border: `1px solid ${C.amber200}`, padding: '5px 12px', borderRadius: 14, color: C.amber700, fontSize: 11, fontWeight: 700 }}>
              <span>⭐</span>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 16, paddingTop: 16, borderTop: `1px solid ${C.slate100}`, fontSize: 11 }}>
            <div style={{ display: 'flex', gap: 16 }}>
              <div>
                ...
                <span style={{ fontWeight: 700, color: C.slate700, fontSize: 14 }}>92 / 100</span>
              </div>
              <div>
                <span style={{ color: C.slate400, display: 'block', fontSize: 10 }}>Ketuntasan</span>
                <span style={{ fontWeight: 700, color: C.slate700, fontSize: 14 }}>{otherCompleted || 12} / {otherTotal || 15} Tugas</span>
              </div>
            </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
        {[
          { label: 'Rata-rata Nilai Tugas', value: avgScore.toFixed(1), unit: '', icon: '📊', iconBg: C.purple50, iconColor: C.brand600, change: '+4.2%', changeColor: C.emerald600, changeText: 'dari bulan lalu' },
          ...
          { label: 'Modul Tuntas', value: `${completedModules}`, unit: `/ ${totalModules} Modul`, icon: '✅', iconBg: C.emerald50, iconColor: C.emerald600, change: `${Math.round((completedModules / totalModules) * 100)}% Tuntas Kurikulum`, changeColor: C.emerald600, changeText: '' },
          { label: 'Predikat Akhir', value: 'Sangat Baik', unit: '', icon: '⭐', iconBg: C.amber50, iconColor: C.amber500, change: '', changeColor: C.slate500, changeText: 'Daya serap di atas target rata-rata', badge: 'A' },
        ].map((stat, i) => (
        ...
        <div style={{ background: C.white, borderRadius: 20, padding: 28, border: `1px solid ${C.slate200}`, boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
            <div>
              ...
              <p style={{ fontSize: 11, color: C.slate500, margin: '4px 0 0' }}>Capaian persentase kompetensi mata pelajaran {childFullName} (Kelas {childGrade} SD)</p>
            </div>
            ...
            {SUBJECT_PERF.map((s, i) => (
              <div key={i}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 11, fontWeight: 600, marginBottom: 6 }}>
                  ...
                  <span style={{ fontWeight: 700, color: s.color, background: s.bg, padding: '3px 10px', borderRadius: 6, border: `1px solid ${s.border}`, fontSize: 11 }}>
                    {s.pct}% ({s.label})
                  </span>
                </div>
                <div style={{ width: '100%', background: C.slate100, height: 14, borderRadius: 999, overflow: 'hidden' }}>
                  <div style={{ width: `${s.pct}%`, height: '100%', borderRadius: 999, background: s.gradient, transition: 'width 0.6s ease' }} />
                </div>
              </div>
            ))}
          </div>
      ...
      <section style={{ background: C.white, borderRadius: 20, border: `1px solid ${C.slate200}`, boxShadow: '0 1px 3px rgba(0,0,0,0.04)', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '16px 20px', background: 'rgba(248,250,252,0.5)', borderTop: `1px solid ${C.slate100}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 11, color: C.slate500 }}>
          <span>Menampilkan <strong>5 dari 24</strong> total tugas semester ini</span>
          <div style={{ display: 'flex', gap: 4 }}>
            ...
            <button style={{ padding: '6px 12px', borderRadius: 8, border: `1px solid ${C.slate200}`, background: C.white, color: C.slate600, fontSize: 11, fontWeight: 500, cursor: 'pointer' }}>2</button>
            <button style={{ padding: '6px 12px', borderRadius: 8, border: `1px solid ${C.slate200}`, background: C.white, color: C.slate600, fontSize: 11, fontWeight: 500, cursor: 'pointer' }}>3</button>
            <button style={{ padding: '6px 12px', borderRadius: 8, border: `1px solid ${C.slate200}`, background: C.white, color: C.slate600, fontSize: 11, fontWeight: 500, cursor: 'pointer' }}>Selanjutnya</button>
          </div>
        </div>
      </section>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.8)', margin: 0, lineHeight: 1.6, maxWidth: 500 }}>
              {childName} berhasil mempertahankan rata-rata nilai <strong>{avgScore.toFixed(1)}</strong>. Berikan ia lencana reward atau tetapkan modul tantangan berikutnya untuk menjaga rasa ingin tahunya tetap tinggi.
            </p>
          </div>
        </div>
