...
export function SummaryScreen({ module: mod, onRestart }: { module: Module; onRestart: () => void }) {
  ...
  const pct = totalPossible ? Math.round((totalCorrect / totalPossible) * 100) : 0;

  return (
    <div className="scene summary-scene">
      <span className="panel-tag">Selesai</span>
      <h2>Modul "{mod.title}" Selesai!</h2>
      <div className="summary-score-ring" style={{ ['--pct' as string]: `${pct}%` }}>
        <span>{pct}%</span>
      </div>
