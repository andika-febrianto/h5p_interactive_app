    ...
    <div className='frame-form-with-preview'>
      <form className='frame-form' onSubmit={handleSubmit}>
        <div className='frame-form-grid'>
          ...
          <label className='auth-field'>
            ...
            <select
              value={d.kind}
              onChange={(e) => update('kind', e.target.value as FrameKind)}
              style={{
                minHeight: '50px',
                padding: '12px',
                borderRadius: '12px',
        ...
        <label className='auth-field'>
          ...
          <input
            ...
            placeholder='Catatan internal, tidak tampil ke murid'
          />
        </label>
        ...
        {d.kind === 'video' && (
          <>
            ...
            <p className='frame-form-hint'>
              Video tidak diunggah lewat form ini (ukurannya besar) — tempel URL
              video yang sudah di-hosting (mis. link .mp4 langsung, bukan
              halaman YouTube).
            </p>
            <FileUploadField
      ...
      <div className='frame-preview-pane'>
        <p className='frame-preview-label'>
          👁 Pratinjau langsung (persis tampilan murid)
        </p>
...
function DragDropEditor({
  ...
  const addZone = () =>
    onZones([...zones, { id: randomId('zone'), label: '', hint: '' }])
  const addItem = () =>
    onItems([
      ...items,
      { id: randomId('item'), label: '', zoneId: zones[0]?.id ?? '' },
    ])
  ...
  return (
    <div className='repeat-block'>
      ...
      <p className='repeat-block-label' style={{ marginTop: 16 }}>
        Item (yang diseret)
      </p>
      {items.map((it, i) => (
      ...
      {zones.length === 0 && (
        <p className='frame-form-hint'>Tambahkan minimal 1 zona dulu.</p>
      )}
    </div>
  )
}
...
function MarkerListEditor({
  ...
  const add = () =>
    onChange([
      ...markers,
      { id: randomId('marker'), timeSec: 0, question: emptyQuestion() },
    ])
      ...
      {markers.map((m, i) => (
        <div className='repeat-item' key={m.id}>
          ...
          <button
            type='button'
            className='btn-secondary btn-small'
            onClick={() => onChange(markers.filter((_, x) => x !== i))}
          >
            Hapus penanda ini
          </button>
          <hr className='repeat-divider' />
    ...
    <div className='repeat-block'>
      ...
      {items.map((it, i) => (
        <div className='repeat-item' key={it.id}>
          ...
          <label className='auth-field'>
            <span>
              Jawaban yang diterima (pisahkan dengan koma jika lebih dari satu)
            </span>
            <input
          ...
          <label className='auth-field'>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                next[i] = {
                  ...next[i],
                  inputType: e.target.value as 'text' | 'number',
                }
                onItems(next)
              }}
            >
